...
- Use files in tabulate format with the Differentially Methylated Regions (DMR) for CG, CHG, and CHH contexts for the Azucena and BGI rice varieties and overlap with the genes, and upstream files from RAPD annotation database, available at https://rapdb.dna.naro.go.jp/download/irgsp1.html. Only genes with at least 60% coverage are considered
- Addd the genes with absolute LFC >= 2 and put in a nested listed with the purpose of identify shared genes
- Perform gene enrichment analyses for the differentially methylated and upregulated or downregulated gene lists 
